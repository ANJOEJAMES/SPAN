...
db.serialize(async () => {
  ...
  // Migration: add is_html col if missing
  try {
    const cols = await all("PRAGMA table_info(posts)");
    if (!cols.find(c => c.name === 'is_html')) {
      await run("ALTER TABLE posts ADD COLUMN is_html INTEGER NOT NULL DEFAULT 0");
      // Set existing seed posts to 1 since they use HTML
      await run("UPDATE posts SET is_html = 1");
    }
  ...
  if (row.count === 0) {
    ...
    await run(
      `INSERT INTO comments (post_id, name, email, message, created_at)
       VALUES (1, 'Scott William', 'scott@example.com', 'This is truly inspiring work. SPAN has made such a difference in our community.', '2024-08-31T10:00:00')`
    );
    await run(
      `INSERT INTO comments (post_id, name, email, message, created_at)
       VALUES (1, 'Liam Irvines', 'liam@example.com', 'Thank you for the detailed coverage. Mental health awareness is so important.', '2024-08-30T15:30:00')`
    );
